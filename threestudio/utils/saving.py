...
class SaverMixin:
    ...
    def save_mesh_npy(self, mesh_v_pos, mesh_t_pos_idx, out_mesh):
        mesh_v_save_path = self.get_save_path(mesh_v_pos)
        mesh_t_save_path = self.get_save_path(mesh_t_pos_idx)
        np.save(mesh_v_save_path, out_mesh.v_pos.detach().cpu().numpy())
        np.save(mesh_t_save_path, out_mesh.t_pos_idx.detach().cpu().numpy())
        return mesh_v_save_path, mesh_t_save_path
