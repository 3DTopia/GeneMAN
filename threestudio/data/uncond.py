...
class RandomCameraDataModuleConfig:
    ...
    head_elevation_range: Tuple[float, float] = (-10, 10)
    head_azimuth_range: Tuple[float, float] = (-90, 90)
    head_camera_distance_range: Tuple[float, float] = (0.3, 0.4)
    half_camera_distance_range: Tuple[float, float] = (0.5, 0.7)
...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    def __init__(self, cfg: Any) -> None:
        ...
        self.head_elevation_range = self.cfg.head_elevation_range
        self.head_azimuth_range = self.cfg.head_azimuth_range
        self.head_camera_distance_range = self.cfg.head_camera_distance_range
        self.half_camera_distance_range = self.cfg.half_camera_distance_range

        # part-aware zoom-in
        self.body_part_ratio = [0.7, 0.1, 0.1, 0.1]
        # self.body_part_ratio = [0.0, 1.0, 0.0, 0.0]
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        # select human part
        if self.cfg.sampling_type != "default":
            # 0: full body, 1: head, 2: upper body, 3: lower body
            human_part = np.random.choice([0, 1, 2, 3], p=self.body_part_ratio)            
            is_head = (human_part == 1)
            if is_head:
                elevation_range = self.head_elevation_range
                azimuth_range = self.head_azimuth_range
                camera_distance_range = self.head_camera_distance_range
            elif human_part in [2, 3]:  # half body, decrease the camera distance
                camera_distance_range = self.half_camera_distance_range
        ...
        if self.cfg.sampling_type != "default":
            data["human_part"] = human_part_out
